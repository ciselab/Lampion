...
def readJsonl_andPrintAllToPython(jsonl_file: str, output_prefix:str = "./output/") -> None:
    """
    This method reads a jsonl_file and creates a Pythonfile for each entry found.
    ...
    :return: nothing, creates one Python file per line found
    """
    counter = 0
...
def write_to_file(line:str, output_prefix: str = "./output/" ) -> None:
    ...
    func = line['func_name'].split('.')[1] if "." in line['func_name'] else line['func_name']
    f = open(output_prefix + "/" + path + "/" + classname + "_" + func + ".py","w")
...
# This is a weird string, but it has to be intended like this to not have (wrong) tabs
    filecontent = f"""'''
...
if __name__ == '__main__':
    ...
    """
    print("Running JsonL to Python ")
    if(len(sys.argv)==1):
        print("received no arguments - trying to default to 'python.jsonl' and writing to 'output'")
        readJsonl_andPrintAllToPython('python.jsonl')
    elif(len(sys.argv)==2):
        print(f"Trying to read {sys.argv[1]}, writing to 'output'")
        readJsonl_andPrintAllToPython(sys.argv[1])
    elif(len(sys.argv)==3):
        print(f"Trying to read {sys.argv[1]}, writing to {sys.argv[2]}")
        readJsonl_andPrintAllToPython(sys.argv[1],sys.argv[2])
    else:
